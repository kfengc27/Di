...
function Circlebar(prefs) {
    ...
    this.initialise = function() {
        that.value = parseInt(attribs.circleStarttime) || parseInt(prefs.maxValue) || 0;
        ...
        that.size = attribs.circleSize || prefs.size || "4rem";
        that.fontSize = attribs.circleFontsize || prefs.fontSize || "20px";
        that.fontColor = attribs.circleFontcolor || prefs.fontColor || "black";
        ...
        that.element.find(".loader-shadow").css("border-width", (that.dialWidth * 1.4) + "px");
    ...
    this.setValue = function(val) {
        text = that.element.find(".text");
        that.value = val;
        var percentage = 100 - ((that.value * 100) / that.maxValue);
        that.renderProgress(percentage);
        text[0].dataset.value = that.value;
        text.html(that.value);
    }
